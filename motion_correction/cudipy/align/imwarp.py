...
from ..align.scalespace import ScaleSpace
from dipy.align import Bunch, VerbosityLevels
...
def get_direction_and_spacings(affine, dim):
    ...
    elif isinstance(affine, cp.ndarray):
...
class DiffeomorphicMap(object):
    def __init__(self,
        ...
        if disp_grid2world is None:
            self.disp_grid2world = None
            self.disp_world2grid = None
        else:
            if isinstance(disp_grid2world, cp.ndarray):
                disp_grid2world = cp.asnumpy(disp_grid2world)
            self.disp_grid2world = disp_grid2world
            self.disp_world2grid = linalg.inv(self.disp_grid2world)
        ...
        if domain_grid2world is None:
            ...
            if isinstance(domain_grid2world, cp.ndarray):
                domain_grid2world = cp.asnumpy(domain_grid2world)
            self.domain_grid2world = domain_grid2world
            self.domain_world2grid = linalg.inv(domain_grid2world)
        ...
        if codomain_grid2world is None:
            ...
            if isinstance(codomain_grid2world, cp.ndarray):
                codomain_grid2world = cp.asnumpy(codomain_grid2world)
            self.codomain_grid2world = codomain_grid2world
            self.codomain_world2grid = linalg.inv(codomain_grid2world)

        if prealign is None:
            self.prealign = None
            self.prealign_inv = None
        else:
            if isinstance(prealign, cp.ndarray):
                prealign = cp.asnumpy(prealign)
            self.prealign = prealign
            self.prealign_inv = linalg.inv(prealign)
    ...
    def interpret_matrix(self, obj):
        ...
        if (obj is None) or isinstance(
            obj, (cp.ndarray, np.ndarray)
        ):  # TODO: allow both CuPy and NumPy?
            return obj
    ...
    def allocate(self):
        ...
        if self.coord_axis == -1:
            shape = tuple(self.disp_shape) + (self.dim,)
        else:
            shape = (self.dim,) + tuple(self.disp_shape)
        self.forward = cp.zeros(shape, dtype=floating)
        self.backward = cp.zeros(shape, dtype=floating)
    ...
    def _warp_forward(self, image, interpolation='linear',
        ...
        if out_shape is None:
            ...
            out_shape = cp.asarray(out_shape, dtype=np.int32)
        ...
        if interpolation == 'nearest':
            if image.dtype is cp.dtype('float64') and floating is cp.float32:
                image = image.astype(floating)
            elif image.dtype is cp.dtype('int64'):
                image = image.astype(cp.int32)
            order = 0
        elif interpolation == 'cubic':
            image = cp.asarray(image, dtype=floating)
            order = 3
        else:
            image = cp.asarray(image, dtype=floating)
            order = 1
    ...
    def expand_fields(self, expand_factors, new_shape):
        ...
        """
        expand_f = vfu.resample_displacement_field

        expanded_forward = expand_f(
            self.forward, expand_factors, new_shape, coord_axis=self.coord_axis
        )
        expanded_backward = expand_f(
            self.backward, expand_factors, new_shape, coord_axis=self.coord_axis
        )

        expand_factors = np.append(expand_factors, [1])
        expanded_grid2world = mult_aff(
            self.disp_grid2world, np.diag(expand_factors)
        )
        expanded_world2grid = linalg.inv(expanded_grid2world)
    ...
    def compute_inversion_error(self, omit_stats=False):
        ...
        """
        Dinv = self.disp_world2grid
        residual, stats = vfu.compose_vector_fields(
            self.backward,
            self.forward,
            None,
            Dinv,
    ...
    def warp_endomorphism(self, phi):
        ...
        compose_f = vfu.compose_vector_fields

        forward, stats = compose_f(
            d1,
            d2,
            None,
            premult_disp,
    ...
    def get_simplified_transform(self):
        ...
        diffeomorphic map.
        """
        simplify_f = vfu.simplify_warp_function
...
_norm_div = cp.ElementwiseKernel(
    in_params="W nrm",
    out_params="W fw_step",
    operation="""if (nrm > 0) { fw_step /= nrm; }""",
    name="cudipy_norm_div",
)
...
class SymmetricDiffeomorphicRegistration(DiffeomorphicRegistration):
    ...
    def update(self, current_displacement, new_displacement,
        ...
        """
        tmp = current_displacement * current_displacement
        tmp = tmp.sum(self.coord_axis)
        cp.sqrt(tmp, out=tmp)
        mean_norm = tmp.mean()
        ...
        self.compose(current_displacement, new_displacement, None,
                     ...
                     omit_stats=True, coord_axis=self.coord_axis)

        return current_displacement, mean_norm
    ...
    def _init_optimizer(self, static, moving,
        ...
        self.compose = vfu.compose_vector_fields
        ...
        prealign_inv = None if prealign is None else linalg.inv(prealign)
    ...
    def _iterate(self):
        ...
        current_disp_world2grid = self.static_ss.get_affine_inv(
            self.current_level
        )
        current_disp_spacing = cp.asarray(
            self.static_ss.get_spacing(self.current_level), dtype=cp.float32
        )
        ...
        fw_step = self.metric.compute_forward()
        ...
        if self.coord_axis == -1:
            nrm = fw_step / current_disp_spacing
        else:
            temp = current_disp_spacing.reshape((-1,) + (1,) * self.dim)
            nrm = fw_step / temp
        nrm *= nrm
        nrm = nrm.sum(self.coord_axis)
        cp.sqrt(nrm, out=nrm)
        nrm = nrm.max()
        if nrm.dtype != fw_step.dtype:
            nrm = nrm.astype(fw_step.dtype, copy=False)

        # if nrm > 0:
        #    fw_step /= nrm
        _norm_div(nrm, fw_step)
        ...
        bw_step = self.metric.compute_backward()
        ...
        if self.coord_axis == -1:
            nrm = bw_step / current_disp_spacing
        else:
            temp = current_disp_spacing.reshape((-1,) + (1,) * self.dim)
            nrm = bw_step / temp
        nrm *= nrm
        ...
        cp.sqrt(nrm, out=nrm)
        nrm = nrm.max()
        if nrm.dtype != bw_step.dtype:
            nrm = nrm.astype(bw_step.dtype, copy=False)

        # if nrm > 0:
        #    bw_step /= nrm
        _norm_div(nrm, bw_step)
        ...
        total_energy = fw_energy + bw_energy
        if isinstance(total_energy, cp.ndarray):
    ...
    def _optimize(self):
        ...
        for level in range(self.levels - 1, -1, -1):
            ...
            if level < self.levels - 1:
                expand_factors = self.static_ss.get_expand_factors(
                    level + 1, level
                )
            ...
            while ((self.niter < self.level_iters[self.levels - 1 - level])
                   and (self.opt_tol < derivative)):
        ...
        verbose = self.verbosity >= VerbosityLevels.DIAGNOSE
        omit_stats = not verbose

        # Reporting mean and std in stats[1] and stats[2]
        residual, stats = self.static_to_ref.compute_inversion_error(omit_stats)

        if verbose:
            logger.info(
                "Static-Reference Residual error: %0.6f (%0.6f)"
                % (stats[1], stats[2])
            )

        residual, stats = self.moving_to_ref.compute_inversion_error(omit_stats)

        if verbose:
            logger.info(
                "Moving-Reference Residual error :%0.6f (%0.6f)"
                % (stats[1], stats[2])
            )
        ...
        if verbose:
            logger.info(
                "Final residual error: %0.6f (%0.6f)" % (stats[1], stats[2])
            )
        if self.callback is not None:
    ...
    def optimize(self, static, moving, static_grid2world=None,
        ...
        self._optimize()
        self._end_optimizer()
        self.static_to_ref.forward = cp.asarray(self.static_to_ref.forward)
        self.static_to_ref.backward = cp.asarray(self.static_to_ref.backward)
        return self.static_to_ref
