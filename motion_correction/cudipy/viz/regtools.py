...
def draw_lattice_2d(nrows, ncols, delta, *, xp=np):
    ...
    """
    lattice = np.full(
        (1 + (delta + 1) * nrows, 1 + (delta + 1) * ncols),
        127,  # Fill the lattice with "white"
        dtype=np.float64,
    )
...
def plot_2d_diffeomorphic_map(
    ...
    **fig_kwargs
    ...
    if isinstance(mapping.forward, cp.ndarray):
        on_gpu = True
        xp = cp
    else:
        on_gpu = False
    ...
    if on_gpu:
        warped_forward = cp.asnumpy(warped_forward)
    ...
    if on_gpu:
        warped_backward = cp.asnumpy(warped_backward)
...
def plot_slices(V, slice_indices=None, fname=None, **fig_kwargs):
    ...
    V = cp.asarray(V, dtype=np.float64)
...
def overlay_slices(
    ...
    **fig_kwargs
    ...
    # Normalize the intensities to [0,255]
    sh = L.shape
    L = cp.asarray(L, dtype=np.float64)
    R = cp.asarray(R, dtype=np.float64)
